...
public:
	...
	//@{
		...
		* \param pReplace If \c true, replace the original geometry with the new triangulated meshes on all the nodes, and delete the original geometry. 
		*                 Otherwise, original geometries are left untouched.
		...
		* \remark The function will still iterate through all geometries regardless if one fails to triangulate, but will return false in that case. This function
		...
		* \param pReplace If \c true, replace the original geometry with the new triangulated mesh on the nodes, and delete the original geometry.
		*                 Otherwise, the original geometry is left untouched, the new mesh is added to the nodes, and becomes the default one.
		* \param pLegacy If \c true, use legacy triangulation method that does not support holes in geometry. Provided for backward compatibility.
		* \param pReplacedAttribs List of all node attributes that have been replaced by the triangulated mesh.
		*                 This list is only filled if pReplace is set to \c true. 
		...
		* \remark If pReplacedAttribs is not defined, this function automatically delete the original node attribute that have been replaced otherwise the list is
		*         filled and it is up to the caller to delete the objects.
		* \remark If the pNodeAttribute is of type eNurbsSurface and is used by an FbxTrimNurbsSurface, the FbxTrimNurbsSurface object is also going to be added to the pReplaced list. */ 
		FbxNodeAttribute* Triangulate(FbxNodeAttribute* pNodeAttribute, bool pReplace, bool pLegacy=false, FbxArray<FbxNodeAttribute*>* pReplacedAttribs = nullptr);
