...
namespace FW
{
	...
	bool ValidateCreateBuffer(const GpuBufferDesc& InBufferDesc, GpuResourceState InitState)
	{
		...
		//	}
		//}
		else if (EnumHasAnyFlags(InBufferDesc.Usage, GpuBufferUsage::Structured | GpuBufferUsage::RWStructured))
		{
			if (InBufferDesc.StructuredInit.Stride == 0 || (InBufferDesc.ByteSize < InBufferDesc.StructuredInit.Stride))
			{
	...
	bool ValidateCreateTexture(const GpuTextureDesc& InTexDesc, GpuResourceState InitState)
	{
		...
		if (InTexDesc.Dimension == GpuTextureDimension::Tex3D && EnumHasAnyFlags(InTexDesc.Usage, GpuTextureUsage::Shared))
		{
			SH_LOG(LogRhiValidation, Error, TEXT("CreateTexture Error(Shared usage is not supported for 3D textures)"));
			return false;
		}
