...
namespace SH
{
	...
	TRefCountPtr<FW::GpuBindGroup> ShaderToyPassNode::GetBuiltInBindGroup(GpuBindGroupLayout* Layout)
	{
	...
	void ShaderToyPassNode::UpdateChannelPinTypes()
	{
		...
		for (int i = 0; i < 4; i++)
		{
			...
			{
				NewPin = NewShObject<GpuTexturePin>(this);
			}
			NewPin->ObjectName = OldPin->ObjectName;
		...
		{
			auto ShEditor = static_cast<ShaderHelperEditor*>(GApp->GetEditor());
			if (auto* GraphPanel = ShEditor->GetGraphPanel())
			{
				if (auto* NodeWidget = GraphPanel->GetNode(this))
				{
					NodeWidget->RebuildPins();
				}
			}
		}
	}
	...
	void ShaderToyPassNode::UpdateBuiltinUniformBuffer(ShaderToyExecContext& Context)
	{
		...
		{
			FString ChannelName = FString::Printf(TEXT("iChannel%d"), i);
			GraphPin* iChannel = GetPin(ChannelName);
			if (iChannel->HasLink())
			{
				GpuTexture* Tex;
        ...
        if(ShaderAssetObj->GetPixelShader()->IsCompiled())
        {
            ...
            if(PassOutput->GetValue()->GetWidth() != (uint32)ShaderToyContext.iResolution.x ||
               PassOutput->GetValue()->GetHeight() != (uint32)ShaderToyContext.iResolution.y ||
				PassOutput->GetValue()->GetFormat() != (GpuFormat)Format)
            {
                GpuTextureDesc Desc{ (uint32)ShaderToyContext.iResolution.x, (uint32)ShaderToyContext.iResolution.y, (GpuFormat)Format, GpuTextureUsage::ShaderResource | GpuTextureUsage::RenderTarget | GpuTextureUsage::Shared };
            ...
            if (static_cast<ShaderToy*>(GetOuter())->FlipY)
            {
                for (int i = 0; i < 4; i++)
                {
                    ...
                    FString ChannelName = FString::Printf(TEXT("iChannel%d"), i);
                    auto* iChannel = static_cast<GpuTexturePin*>(GetPin(ChannelName));
                    if (iChannel->HasLink())
                    {
